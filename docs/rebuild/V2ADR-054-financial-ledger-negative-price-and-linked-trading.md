...
The third planner is named **MEP — Markt Etmaal Planner**. All three planners
consume the same immutable `PlanningInputSnapshot`, but no planner may consume,
query or restore another planner's candidates, selected windows, observations,
commitments or persisted results. Shared physical and settlement libraries are
pure calculation code and are not planner-to-planner communication.

The three temporary comparison roles are explicit:
...
- MEP independently generates and simulates its own PV/NOM, necessary
  grid-acquisition, negative-price and storage-export routes and is the sole
  live planner;
...
MEP may receive live authority only after the acceptance suite proves its
complete native route remains safe when no optional market route is decisive.
A runtime guard must make simultaneous dispatch authority impossible.
