...
## 2026-09-09 — dev.244 live corrections: retained dashboard and published market reference

User report and authorization: the initial broad NOM window disappears and the dashboard appears to show discharge-only all day; "akkoord maak fix". Baseline main c54e28e5a79b7a716bc3e38fe8222de4528df8d0, tree 8e559f550bf6bf73f0b99b22e59d9f245b49972e. Branch fix/dev244-retained-plan-and-market-prices. ADR001–037 and accepted session addenda remain unchanged.

Evidence from picot-diagnostics-2026-09-09T065215.314.zip: dev.244 binds plan-5829f9b94e21e3f8 at 06:44 local, with NOM 08:30–17:45 local. Every later recorded poll through 06:51 retains that exact plan. The dashboard filters execution plans against the current poll's winning candidate, which is intentionally absent for plan_retained; reproduction showed one stored plan but zero displayed plans and no SOC curve. Discharge-only before 08:30 is expected; the supplied archive does not prove subsequent live dispatch. Separately, all 96 day-price quarters exist in HA evidence, while the live snapshot keeps only 69 unexpired quarters. Requesting a full-day market tariff from this filtered snapshot deterministically raises daily_tariff_price_coverage_incomplete.

Changes: render retained execution plans with original plan/candidate/path identities and main NOM charge bounds. Keep the original canonical SOC projection in the existing WebViewStore only while the same immutable plan/path remains retained; display its original calculation time. Never re-simulate in UI, attach another plan's curve, or carry through fallback. After process restart without a previously published projection, the retained windows remain visible but no SOC curve is invented. Complete published_price_points are now separately carried in immutable Planning Input. Explicit tariff reference windows use this evidence, including elapsed quarters; future-only price_points and normal executable opportunity horizons remain unchanged. Missing publication quarters still block admission.

Validation: original dashboard failure reproduced before change; original full-day versus filtered-price coverage reproduced directly from uploaded evidence. 92 affected tests passed in 71.04s, plus the new published-reference boundary test. After the SOC display correction, 48 affected UI/retention/reference tests passed in 5.48s. Ruff passed and mypy passed for all 68 v2 modules. Local mypy initially failed due to a malformed SQLite cache; using an isolated cache resolved the tool failure without source changes. No HA commands, release version bump, or deployment performed by this fix. CI result is recorded on the pull request.
