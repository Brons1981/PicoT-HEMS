...
(async () => {
  ...
  try {
    ...
    for(let day=0;day<7;day++){
      ...
      await row.locator('[name=start]').fill('18:00');
      await row.locator('[name=end]').fill('22:00');
      await row.locator('[name=target]').fill('20');
      await row.locator('[name=hard]').check();
      assert.equal(await row.locator('[name=minimum]').inputValue(),'20');
      await row.locator('[name=maximum]').fill('20.5');
    }
    await schedule.getByRole('button',{name:'Venster toevoegen'}).click();
    ...
    await night.locator('[name=minimum]').fill('17');
    await night.locator('[name=maximum]').fill('18');
    await page.evaluate(()=>refresh());
    assert.equal(await schedule.locator('.schedule-row').count(),8,'Refresh retains unsaved windows');
    ...
    await page.reload();
    await page.waitForFunction(()=>document.querySelectorAll('.schedule-row').length===8);
