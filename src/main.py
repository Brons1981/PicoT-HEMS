...
from capability_semantic_validation import validate_capability_candidates
...
def main() -> None:
    """Run Discovery through semantic validation and Capability Selection."""
    try:
        ...
        semantic_result = validate_capability_candidates(capability_result)
        semantic_files = export_capability_semantic_validation(
            semantic_result,
            OUTPUT_DIRECTORY,
        )

        selection_result = select_capabilities(semantic_result)
    ...
    print("PicoT Discovery Step 2.7 with semantic validation completed.")
    ...
    print(f"Semantically valid candidates: {semantic_summary['valid_candidate_count']}")
    print(f"Semantically rejected candidates: {semantic_summary['rejected_candidate_count']}")
    print(f"Semantic statuses: {semantic_summary['status_counts']}")
    ...
    print(f"Semantic validation summary: {semantic_files['summary']}")
