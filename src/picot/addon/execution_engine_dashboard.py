...
def execution_engine_dashboard_state(event: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "attributes": {
            ...
            "execution_control_authority": event.get(
                "execution_control_authority", "unverified"
            ),
            "execution_authority_verified": event.get(
                "execution_authority_verified", False
            ),
            "execution_request_count": event.get("execution_request_count", 0),
            "execution_raw_request_count": event.get("execution_raw_request_count", 0),
            "execution_authority_suppressed_count": event.get(
            ...
            "execution_adapter_invoked": event.get("execution_adapter_invoked", False),
            "execution_dispatch_attempted": event.get(
