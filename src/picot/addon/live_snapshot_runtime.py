...
def household_state_from_telemetry(event: dict[str, object]) -> HouseholdState:
    ...
    # Household load is a genuine PicoT-owned derivation from the three selected
    # authoritative power roles. It is only calculated when every term exists.
    household_load: float | None = None
    if grid_power is not None and pv_power is not None and battery_power is not None:
        # Zendure signed power is positive while charging. Positive grid means
        # import. Therefore load = PV + grid - battery charging power.
