...
from picot.addon.planning_snapshot_assembly import NormalizedPlanningInputs, assemble_planning_input_snapshot
...
from picot.domain.pv_energy_timeline import PVEnergyEvidenceType, PVEnergyTimeline, PVEnergyTimelineInterval
...
def current_storage_state_from_telemetry(event: dict[str, object], *, sequence: int) -> CurrentStorageState | None:
...
def pv_energy_timeline_from_telemetry(event: dict[str, object], *, sequence: int, captured_at: datetime) -> PVEnergyTimeline | None:
    raw_points = event.get("solcast_today_forecast_points", [])
    raw_tomorrow = event.get("solcast_tomorrow_forecast_points", [])
    if not isinstance(raw_points, list) or not isinstance(raw_tomorrow, list):
        return None
    parsed: list[tuple[datetime, float]] = []
    for item in (*raw_points, *raw_tomorrow):
        if not isinstance(item, dict):
            continue
        start = item.get("period_start")
        estimate = item.get("pv_estimate")
        if not isinstance(start, str) or isinstance(estimate, bool) or not isinstance(estimate, (int, float)):
            continue
        moment = datetime.fromisoformat(start.replace("Z", "+00:00"))
        if moment.tzinfo is None or moment.utcoffset() is None:
            continue
        parsed.append((moment, max(0.0, float(estimate))))
    parsed.sort(key=lambda item: item[0])
    if len(parsed) < 2:
        return None

    intervals: list[PVEnergyTimelineInterval] = []
    for (start, estimate_kw), (next_start, _) in zip(parsed, parsed[1:], strict=False):
...
def build_live_planning_snapshot(event: dict[str, object], *, sequence: int, horizon_hours: int = 24) -> PlanningInputSnapshot:
    if sequence < 1:
    ...
    pv_timeline = pv_energy_timeline_from_telemetry(event, sequence=sequence, captured_at=captured_at)
    horizon_end = pv_timeline.horizon_end if pv_timeline is not None else captured_at + timedelta(hours=horizon_hours)
...
def snapshot_log_event(snapshot: PlanningInputSnapshot) -> dict[str, object]:
    ...
    return {
        ...
        "pv_timeline_energy_wh": snapshot.pv_energy_timeline.total_energy_wh if snapshot.pv_energy_timeline is not None else None,
