...
def run_telemetry_once(options: dict[str, Any], token: str, planner_event: dict[str, object]) -> dict[str, object]:
    ...
    while True:
        ...
        if scheduled_boundary is not None and wall_clock_now >= scheduled_boundary.occurs_at:
            try:
                ...
                planner_event = runtime.run_scheduled_boundary_once(options, token, planner_event, scheduled_boundary, now=wall_clock_now)
                runtime._log_event(planner_event)
                scheduled_boundary = runtime._scheduled_boundary(planner_event, now=wall_clock_now)
            except Exception as exc:
        ...
        if planner_event is not None and monotonic_now >= next_telemetry_run:
            try:
                ...
                runtime._log_event(pv_forecast_comparison_log_event(telemetry_event))
            except Exception as exc:
