"""Frozen pre-MEP CP pipeline retained only for historical regression tests.

This module is not part of the production package and has no runtime authority.
"""
...
class CanonicalPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        market_daily_planner_runtime: MarketDailyPlannerRuntime | None = None,
    ...
    def _execute(
        ...
        *,
        ...
        if self._market_daily_planner_runtime is not None:
            run, mep_timings, _ = build_mep_canonical_run(
                snapshot=snapshot,
                opportunities=opportunities,
                planner_runtime=self._market_daily_planner_runtime,
                commitment_store=self._commitment_store,
                control_change_allowed=control_change_allowed,
                switching_margin_eur=0.05,
            )
            return run, PipelineStageTimings(
                opportunity_engine_ms=opportunity_engine_ms,
                candidate_engine_ms=mep_timings.candidate_engine_ms,
                evaluation_engine_ms=mep_timings.evaluation_engine_ms,
                execution_plan_builder_ms=(
                    mep_timings.execution_plan_builder_ms
                ),
                execution_engine_ms=mep_timings.execution_engine_ms,
                execution_primitive_ms=mep_timings.execution_primitive_ms,
                device_adapter_ms=mep_timings.device_adapter_ms,
                vendor_result_ms=mep_timings.vendor_result_ms,
                canonical_total_ms=round(
