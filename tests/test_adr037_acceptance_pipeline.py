...
def _capabilities(*, maximum_power_w: float = 2000.0) -> CapabilitySnapshotSet:
    return CapabilitySnapshotSet(
        snapshot_id="snapshot-adr037-acceptance",
        mapping_version=1,
        captured_at=BASE,
        capabilities=(_capability(maximum_power_w=maximum_power_w),),
    )
...
def _no_opportunities() -> OpportunitySet:
    return OpportunitySet(
        snapshot_id="snapshot-adr037-acceptance",
        opportunities=(),
    )
...
def test_pv_sufficient_path_needs_no_grid_supported_candidate() -> None:
    result = _run(
        balance=_pv_sufficient_balance(),
...
def test_pv_shortfall_makes_grid_supported_path_the_only_valid_winner() -> None:
    ...
    cost_paths = [
        path for path in result.candidate_set.energy_paths if path.family.value == "cost_first"
    ]
    ...
    validity = {
        item.candidate_id: item.validity for item in result.candidate_outcomes.outcomes
    }
    baseline = next(
        candidate
