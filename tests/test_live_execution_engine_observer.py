...
def test_empty_baseline_plan_set_is_evaluated_without_requests_or_records() -> None:
    ...
    assert fields["execution_authority_verified"] is False
    assert fields["execution_request_count"] == 0
    assert fields["execution_raw_request_count"] == 0
    assert fields["execution_record_count"] == 0
    assert fields["execution_adapter_invoked"] is False
    assert fields["execution_dispatch_attempted"] is False
